...
mice.impute.2l.heckman1step_ord_re_ghq <- function(y, ry, x, type, QP=rep(10,2), draw=T, excl="",...) { 
  ...
  cov_mode <- lapply(1:nrow(modes),function(x) make.positive.definite(solve(hess_mode[[x]],tol = 1e-300)))
...
mice.impute.2l.heckman1step_ord_re_ghq <- compiler::cmpfun(mice.impute.2l.heckman1step_ord_re_ghq)
